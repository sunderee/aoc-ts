import { type Point2D, Point2DMap, Point2DSet, type Solution } from "../../common";
...
export class Day06Year2015 implements Solution {
    ...
    private executeLightCommand(command: ParsedCommand, currentState: Point2DSet): void {
    ...
    private executeBrightnessCommand(command: ParsedCommand, brightnessMap: Point2DMap<number>): void {
        for (let x = command.from.x; x <= command.to.x; x++) {
            for (let y = command.from.y; y <= command.to.y; y++) {
                const point = { x, y };
    ...
    private parseCommand(line: string): ParsedCommand {
        const commandString: string | undefined = line.match(/^(turn on)|(turn off)|(toggle)/m)?.[0];

        const coordinatesArray: Point2D[] | undefined = line.match(/(\d{1,3},\d{1,3})/g)?.map((match) => {
        ...
        if (commandString === undefined || fromCoordinates === undefined || toCoordinates === undefined) {
